...
logFolder = 'logfiles/hidden100_profile/';
...
plotColors = colormap(jet(maxN+1));
...
elseif(strcmp(performanceMetric, 'MSE'))
    ylabelString = {'Multitasking Accuracy', 'in % (MSE)'};
    ylim([0 0.08]);
...
%% giovanni plot
allMIS = unique(MISData_specific);

x_data = nan(length(allMIS),1);
y_mean = nan(length(allMIS),1);
y_sem = nan(length(allMIS),1);

misCol = 3;

for misIdx = 1:length(allMIS)
    mis = allMIS(misIdx);
    idx = find(MISData_specific == mis);
    x_data(misIdx) = mis;
    y_mean(misIdx) = mean(accuracyCurve(idx,misCol));
    y_sem(misIdx) = std(accuracyCurve(idx,misCol)) / sqrt(length(idx));
end
errorbar(x_data, y_mean, y_sem, 'LineWidth', 3);
ylabel('Best MIS Performance');
xlabel('MIS');
set(gca, 'fontSize', 16);
...
for threshIdx = 1:length(plotted_correlation_thresholds)
    ...
    xlabel('Predicted MIS', 'FontSize', fontSize_ylabel-2);
...
%% FIG 2: confusion matrix for particular optimal control policy performance vs. extracted MIS

fig2 = figure(2);
set(fig2, 'Position', [100 100 400 400]);

true_ParallalProcessingCapablity =  data_optimalControlPolicy_meanAcc(:,:,optPerformanceThreshIdx);
true_ParallalProcessingCapablity = true_ParallalProcessingCapablity(:);

extracted_ParallalProcessingCapablity = data_extractedMIS(:, :, corrThreshIdx);
extracted_ParallalProcessingCapablity = extracted_ParallalProcessingCapablity(:);

confusionMatrix = zeros(NPathways, NPathways);
for i =1:size(true_ParallalProcessingCapablity,1)
    confusionMatrix(true_ParallalProcessingCapablity(i), extracted_ParallalProcessingCapablity(i)) = confusionMatrix(true_ParallalProcessingCapablity(i), extracted_ParallalProcessingCapablity(i))  + 1;
end
confusionMatrix = confusionMatrix ./ size(true_ParallalProcessingCapablity,1) * 100;

imagesc(confusionMatrix); hold on;
pbaspect([NPathways NPathways 1]);
colorbar;
colormap(cool);
caxis([0 100]);
set(gca, 'XTick', 1:NPathways, 'XTickLabel',  '');
set(gca, 'YTick', 1:NPathways, 'YTickLabel',  '');
set(gca,'xtick', linspace(0.5,NPathways+0.5,NPathways+1), 'ytick', linspace(0.5,NPathways+.5,NPathways+1));
set(gca,'xgrid', 'on', 'ygrid', 'on', 'gridlinestyle', '-', 'xcolor', 'k', 'ycolor', 'k');
xlabel('Optimal Amount of Parallel Processing', 'FontSize', fontSize_xlabel);
ylabel('Extracted MIS', 'FontSize', fontSize_ylabel);
title(['Corr Thresh = ' num2str(correlation_threshold) ', Perf Thresh = ' num2str(optimalPerformanceThreshold)], 'FontSize', fontSize_title);
set(gcf, 'Color', 'w');

% plot cell content
for row = 1:size(confusionMatrix,1)
    for col = 1:size(confusionMatrix,2)
        text(row-0.25,col,strcat(num2str(confusionMatrix(row, col)), '%'), 'FontSize', fontSize_gca);
    end
end
% plot diagonal grid
for diag = 1:NPathways
    plot([0.5 1.5]+(diag-1), [0.5 0.5]+(diag-1), '-k', 'LineWidth',3);
    plot([0.5 1.5]+(diag-1), [1.5 1.5]+(diag-1), '-k', 'LineWidth',3);
    plot([0.5 0.5]+(diag-1), [0.5 1.5]+(diag-1), '-k', 'LineWidth',3);
    plot([1.5 1.5]+(diag-1), [0.5 1.5]+(diag-1), '-k', 'LineWidth',3);
end


%% FIG 3: Computation Times
fontSize = 14;

fig3 = figure(3);
set(fig3, 'Position', [50 100 1200 600]);
subplot(2,1,1);

y = [mean(timing_spec_findMIS(:)) mean(timing_spec_trainOnline(:)) mean(timing_spec_findOptimalControlPolicy(:)) mean(timing_spec_extractMIS(:))];
sem = [std(timing_spec_findMIS(:)) std(timing_spec_trainOnline(:)) std(timing_spec_findOptimalControlPolicy(:)) std(timing_spec_extractMIS(:))] ./sqrt(length(timing_spec_findMIS(:)));

bar(y); hold on;
errorbar(y, sem);

ylabel('Computation Time in Seconds', 'fontSize', fontSize);
title('Computational Efficiency', 'fontSize', fontSize);
set(gca, 'XTickLabel', {'MIS Computation', 'Network Training', 'Find Optimal Control Policy', 'Extract MIS From Trained Network'});
set(gca, 'fontSize', fontSize);

subplot(2,1,2);
y = [mean(timing_findMIS(:)) mean(timing_findOptimalControlPolicy(:)) mean(timing_extractMIS(:))];
sem = [std(timing_findMIS(:)) std(timing_findOptimalControlPolicy(:)) std(timing_extractMIS(:))] ./sqrt(length(timing_findMIS(:)));
...
ylabel('Computation Time in Seconds', 'fontSize', fontSize);
title('Computational Efficiency', 'fontSize', fontSize);
set(gca, 'XTickLabel', {'MIS Computation: 1x', ...
                                ['Find Optimal Control Policy: ' num2str(round(mean(timing_findOptimalControlPolicy(:)) / mean(timing_findMIS(:)))) 'x'], ...
                                ['Extract MIS From Trained Network ' num2str(round(mean(timing_extractMIS(:)) / mean(timing_findMIS(:)))) 'x']});
set(gca, 'fontSize', fontSize);
...
optimalPerformanceThreshold = 0.90; % threshold on performance to decide whether the multitasking attempt was successful or not (only necessary for effective PPC metric)
cardinality_window = -3:3;  % plotted cardinality relative to predicted MIS
performanceMetric = 'MSE';         % this could be absoluteAccuracy, PCorrect, LCA
performanceType = 'max';                                % this could be min, max, effective
graphIDselected = 200;
...
% plot all performance curves
fig1 = figure(1);
set(fig1, 'Position', [500 500 400 250]);
plotColors = colormap(jet(maxN));

graphIdx = graphIDselected;
    plot(accuracyCurve(graphIdx,:) * 100, '-', 'LineWidth', lineWidth-2, 'color', plotColors(MISData_specific(graphIdx),:)); hold on;
...
title([performanceMetric ' ' performanceType], 'FontSize', fontSize_title);
ylabel('Performance (%)', 'FontSize', fontSize_ylabel);
xlabel('Predicted MIS', 'FontSize', fontSize_ylabel);
set(gca, 'FontSize', fontSize_gca);

function [outfile] = Giovanni_Sim1_V8_accuracyCheck_fun(inputArg, hiddenArg, silence)
...
%         MultiTask= sum(findMIS(logical(A_dual),[1:m]));
        ...
        [taskCardinality, maximumPerformanceCurve, minimumPerformanceCurve, effectivePPC, effectivePPC_unthresholded, timing_policy] = findOptimalControlPolicy_4_accuracyCheck(taskNet, NPathways, multiCap_inc, goodPerformanceThresholds);
        ...
        maximumPerformanceCurveLog.CorrectResponseAct(rep,:) = maximumPerformanceCurve.CorrectResponseAct;
        ...
        minimumPerformanceCurveLog.CorrectResponseAct(rep,:) = minimumPerformanceCurve.CorrectResponseAct;
    ...
    if(isa(inputArg, 'char'))
        outfile = ['logfiles/Giovanni_Simulation_V8_accuracyCheck_h' num2str(nHidden) '_' graphSpecsFolder '_' samplingFolder '_A' num2str(A_ID) '_' num2str(log_version) '.mat'];
        save(outfile);
    end
end
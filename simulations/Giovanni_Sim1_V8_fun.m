function [outfile] = Giovanni_Sim1_V8_fun(inputArg, hiddenArg, silence)
...
%     MultiTask= sum(findMIS(logical(A_dual),[1:m]));
    ...
    if(fixWeights == 1)
            ...
            elseif(strcmp(fixedWeightPattern, 'tensor'))
                outfile = ['logfiles/Giovanni_Simulation_V8_fixedTensor_h' num2str(nHidden) '_' graphSpecsFolder '_' samplingFolder '_A' num2str(A_ID) '_' num2str(log_version) '.mat'];
            end
        elseif(fixWeights == 0)
            outfile = ['logfiles/Giovanni_Simulation_V8_freeWeights_h' num2str(nHidden) '_' graphSpecsFolder '_' samplingFolder '_A' num2str(A_ID) '_' num2str(log_version) '.mat'];
        end

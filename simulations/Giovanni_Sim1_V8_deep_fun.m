function [outfile] = Giovanni_Sim1_V8_deep_fun(inputArg, hiddenArg, silence)

addpath('/home/dmturner/RTDist_1.0.0-rc2/MATLAB');

numLayers = 2;
hiddenArgNum = str2num(hiddenArg);
nHidden = repmat(hiddenArgNum, 1, numLayers);
...
%     MultiTask= sum(findMIS(logical(A_dual),[1:m]));
    ...
    if(fixWeights == 1)
            if(strcmp(fixedWeightPattern, 'basis'))
                outfile = ['logfiles/Giovanni_Simulation_V8_deep_h' num2str(nHidden(1)) '_' graphSpecsFolder '_' samplingFolder '_A' num2str(A_ID) '_' num2str(log_version) '.mat'];
            elseif(strcmp(fixedWeightPattern, 'tensor'))
                outfile = ['logfiles/Giovanni_Simulation_V8_deep_fixedTensor_h' num2str(nHidden(1)) '_' graphSpecsFolder '_' samplingFolder '_A' num2str(A_ID) '_' num2str(log_version) '.mat'];
            elseif(strcmp(fixedWeightPattern, 'random'))
                outfile = ['logfiles/Giovanni_Simulation_V8_deep_fixedRandom_h' num2str(nHidden(1)) '_' graphSpecsFolder '_' samplingFolder '_A' num2str(A_ID) '_' num2str(log_version) '.mat'];
            end
        elseif(fixWeights == 0)
            outfile = ['logfiles/Giovanni_Simulation_V8_deep_freeWeights_h' num2str(nHidden(1)) '_' graphSpecsFolder '_' samplingFolder '_A' num2str(A_ID) '_' num2str(log_version) '.mat'];
        end
...
%         MultiTask= sum(findMIS(logical(A_dual),[1:m]));
        ...
        if(fixWeights == 1)
            if(strcmp(fixedWeightPattern, 'basis'))
                fixToBasisSet = 1;
                taskNet = initializeNetworkToGraph(A, taskNet, 0, 1, fixToBasisSet);
            elseif(strcmp(fixedWeightPattern, 'tensor'))
                fixToBasisSet = 0;
                taskNet = initializeNetworkToGraph(A, taskNet, 0, 1, fixToBasisSet);
            elseif(strcmp(fixedWeightPattern, 'random'))
                fixToBasisSet = 2;
                [taskNet, fixToBasisSet_log]= initializeNetworkToGraph(A, taskNet, 0, 1, fixToBasisSet);
            end
        end
        ...
        [taskCardinality, maximumPerformanceCurve, minimumPerformanceCurve, effectivePPC, effectivePPC_unthresholded, timing_policy] = findOptimalControlPolicy_4(taskNet, NPathways, multiCap_inc, goodPerformanceThresholds);
        ...
        for corr_Idx = 1:length(correlation_thresholds)
            ...
            for layer = 1:length(taskNet.Nhidden)
                Sim1 = hiddenSimilarity{layer};
                if(layer >= length(taskNet.Nhidden))
                    Sim2 = outputSimilarity;
                else
                    Sim2 = hiddenSimilarity{layer+1};
                end
                [~, maxCarryingCapacity] = getMaxCarryingCapacity(Sim1, Sim2, corr_threshold);
                extractedMISLog.MIS(rep, corr_Idx, layer) = str2num(maxCarryingCapacity);
            end
        end
        ...
        minExtractedMIS(rep, :) = min(extractedMISLog.MIS(rep, :), [], 3);
    ...
    if(isa(inputArg, 'char'))
        if(fixWeights == 1)
            if(strcmp(fixedWeightPattern, 'basis'))
                outfile = ['logfiles/Giovanni_Simulation_V8_deep_h' num2str(nHidden) '_' graphSpecsFolder '_' samplingFolder '_A' num2str(A_ID) '_' num2str(log_version) '.mat'];
            elseif(strcmp(fixedWeightPattern, 'tensor'))
                outfile = ['logfiles/Giovanni_Simulation_V8_deep_fixedTensor_h' num2str(nHidden) '_' graphSpecsFolder '_' samplingFolder '_A' num2str(A_ID) '_' num2str(log_version) '.mat'];
            elseif(strcmp(fixedWeightPattern, 'random'))
                outfile = ['logfiles/Giovanni_Simulation_V8_deep_fixedRandom_h' num2str(nHidden) '_' graphSpecsFolder '_' samplingFolder '_A' num2str(A_ID) '_' num2str(log_version) '.mat'];
            end
            
        elseif(fixWeights == 0)
            outfile = ['logfiles/Giovanni_Simulation_V8_deep_freeWeights_h' num2str(nHidden) '_' graphSpecsFolder '_' samplingFolder '_A' num2str(A_ID) '_' num2str(log_version) '.mat'];
        end
        save(outfile);
    end
end